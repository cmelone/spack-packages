...
- If a reference recipe is provided, use it to inform best practices and additional features, but do not copy behavior that is not backed by the extracted metadata
...
If a git URL was used and no versions were detected, ensure the recipe has `git = "<url>"` set. Do not spend time trying to populate versions — the user can supply tarballs later for Spack to checksum and add as proper versioned entries.
