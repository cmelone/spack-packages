...
class CMakeContext:
    ...
    def format_ifs(self, if_blocks):
        def flatten_if(if_block):
            ...
            nested_false = if_block["if_false"].get("ifs", []) if if_block["if_false"] else []
            if nested_true:
...
def extract_from_ast(nodes, context):
    ...
    for node in nodes:
        items = [token.value for token in node.args] if hasattr(node, "args") and node.args else []
...
def strip_at_lines(text):
    lines = text.split("\n")
    filtered_lines = [
        line for line in lines if not (line.strip().startswith("@") and line.strip().endswith("@"))
    ]
    return "\n".join(filtered_lines)
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Extract CMake metadata from a directory")
